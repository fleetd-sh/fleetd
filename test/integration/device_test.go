...
func TestDeviceRegistrationFlow(t *testing.T) {
	...
	err = db.QueryRowContext(context.Background(),
		"SELECT last_seen FROM devices WHERE id = ?",
		regResp.Msg.DeviceId).Scan(&lastSeen)
	require.NoError(t, err)
	assert.True(t, lastSeen.Valid)
	...
	err = db.QueryRowContext(context.Background(),
		"SELECT name, type, version, metadata FROM devices WHERE id = ?",
