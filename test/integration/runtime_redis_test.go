...
func TestRedisDeployment(t *testing.T) {
	...
	if err := rt.Deploy("redis", bytes.NewReader(redisData)); err != nil {
		t.Fatalf("Failed to deploy Redis: %v", err)
	}

	// Start Redis with config
	config := []string{"--port", "6380", "--save", ""} // Non-default port, disable persistence
	if err := rt.Start("redis", config, &runtime.Config{}); err != nil {
	...
	defer func() {
		procs, err := rt.List()
		if err != nil {
			t.Logf("Failed to list processes: %v", err)
		} else {
			t.Logf("Processes: %v", procs)
		}

		if err := rt.Stop("redis"); err != nil {
			t.Logf("Failed to stop Redis: %v", err)
		}
	...
	time.Sleep(100 * time.Millisecond)
	running, err := rt.IsRunning("redis")
	if err != nil {
